...
//
// ppdustjs-filters-secure - Additional functionality for dustjs-linkedin package v0.0.1
//
...
(function(dust) {
    /*
       ...
       * @module ppdustjs-filters-secure
    ...
    function isValidBaseUrl(s) {
        return s.match(/^(https?:\/\/)?[a-z0-9+:_\/.@#\-]+[?]?$/i);
    }
    ...
    (function (undefined) {
        ...
        for (i = 0x20; i < 128; i += 1) {
            ...
            if (!index[c] && c.match(re)) {
                index[c] = '&#x' + i.toString(16).toUpperCase() + ';';
            }
        }
    ...
    function escapeURL(s) {
        ...
        if (type === 'string') {
            ...
            if (pos === -1) {
                return isValidBaseUrl(s) ? s : escapeHTMLAttribute(s);
            }
            temp_str = s.substring(0, pos + 1);
            out += isValidBaseUrl(temp_str) ? temp_str : escapeHTMLAttribute(temp_str);
